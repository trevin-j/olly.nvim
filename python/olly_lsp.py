...
def init_log(level=logging.WARNING):
    LOG_FILE = os.path.expanduser("~/.cache/nvim/olly.nvim.log")
...
class OllamaServer:
    ...
    def on_initialize(self, params: types.InitializeParams):
        ...
        self.engine = CompletionEngine(
            ...
            fill_in_middle=init_options.get("fill_in_middle")
        )
        self.stream_suggestion = params.initialization_options.get(
    ...
    async def on_completion(self, params: types.CompletionParams):
        ...
        document = self.server.workspace.get_text_document(
            params.text_document.uri)
        lines = document.lines
        suggestion_stream = async_generator_wrapper(self.engine.complete(
            lines, params.position.line, params.position.character))

        self.curr_suggestion = {'line': params.position.line + 1,
                                'character': params.position.character, 'suggestion': ''}
        timing_str = ''
    ...
    def on_change(self, params: types.DidChangeTextDocumentParams):
        ...
        lines = self.server.workspace.get_text_document(
        ...
        if change.text == self.curr_suggestion['suggestion'][0:len(change.text)] and len(change.text) > 0:
            self.curr_suggestion['suggestion'] = self.curr_suggestion['suggestion'][len(
                change.text):]
            ...
            self.curr_suggestion = {'line': 1,
                                    'character': 0, 'suggestion': ''}
            self.clear_suggestion()
            # Trigger a new completion
            position = types.Position(
                line=change.range.end.line, character=change.range.end.character + 1)
    ...
    def clear_suggestion(self):
        self.server.send_notification(
            '$/clearSuggestion', {'message': "clear current"})
